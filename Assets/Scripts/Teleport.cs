...
public class Teleport : MonoBehaviour, IGvrGazeResponder {
  ...
  public void TeleportRandomly() {
	...
	public void TeleportToReticle() {
		var currentPosition = camera.transform.position;
		Ray ray = camera.ViewportPointToRay(new Vector3(0.5F, 0.5F, 0));
		RaycastHit hit;
		if (Physics.Raycast (ray, out hit)) {
			if (hit.distance < 10) {
				var newPosition = ray.GetPoint (hit.distance);
				camera.transform.position = new Vector3 (newPosition.x, currentPosition.y, newPosition.z);
			}
		}
	}
